...
export class Clash {
	 ...
	 */
	addProxiesToGroup = (group,prefix = [],suffix =[]) => {
		const { source } = this;
	 ...
	 */
	replaceGroupTo = (groupName = 'DIRECT',target) => {
		const { source } = this;
		if(_.isArray(source['proxy-groups'])){
	...
	addRule = (type:keyof typeof RuleType,value:string,groupName:string,tail = false) => {
		...
		if(type==='MATCH'){
			...
			rules.push( [type,groupName] as RuleObject );
		}else if(tail){
			if(this.checkExistMatchRule(rules)){
				rules.splice(rules.length - 2,0,[type,value,groupName] as RuleObject);
			}
		}else {
			rules.unshift([type,value,groupName] as RuleObject)
		}
		this.source.rules = rules.map(converters.ruleObjectToRuleStr);
		return this;
	}
	 ...
	 */
	addRulesToGroup = (groupName:string,newRules:[keyof typeof RuleType,string][]) => {
		const readyToAddRules = [];
		const rules =  converters.rulesStrToRulesObject(this.source.rules);
		newRules.forEach( ( [ newRuleType , newValue ] ) => {
...
export class Group implements CGroup{
	...
	constructor( conf:Partial<CGroup> ) {
		Object.assign( (this as any) , this.#defaultConf , conf );
	}
